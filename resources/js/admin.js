...
(function () {
    ...
    const Events = (function () {
        ...
        async function updateGeneral() {
            ...
            let html = generalEvents.reduce(function (res, event) {
                return res + '<li class="nav-item">' +
                    '<a href="javascript:void(0)" class="nav-link' + (selectedEvent === event.id ? ' active' : '') + '" data-id="' + event.id + '">' +
                    '<p>' + event.header + '</p>' +
                    '</a>' +
                    '</li>';
        ...
        function addEventClickHandlers($container) {
            $container.find('.nav-link').off('click').on('click', async function () {
                ...
                selectedEvent = $target.data('id');
                $('.nav-link').filter((i, link) => $(link).data('id') === selectedEvent).addClass('active');

                clearInterval(interval);

                Loader.show();
                $eventContainer.hide();
                await fillEventInfo(selectedEvent);
                $eventContainer.show();
                Loader.hide();
    ...
    function updateEvents() {
        interval = setInterval(function () {
            Events.updateGeneral();
            Events.updateUsers();
            const selectedEvent = Events.getSelectedEvent();
            if (selectedEvent) {
                Events.fillEventInfo(selectedEvent);
            }
        }, 30000);
    }
})();
